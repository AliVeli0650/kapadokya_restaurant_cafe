...
export default function IncomePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      const { error } = await supabase.from('income_transactions').insert({ source_id: selectedSource, amount: amountNum, description: description.trim() || null, transaction_date: selectedDate });
      ...
      <div className="container mx-auto px-6 lg:px-12 max-w-7xl">
        ...
        {showForm && (
          <div className="bg-white border border-gray-200 p-8 mb-8">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div><label className="block text-sm uppercase tracking-wide text-gray-700 mb-2">Tutar (€) *</label><input type="number" step="0.01" min="0" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0.00" className="w-full border border-gray-300 px-4 py-3 focus:outline-none focus:border-gray-900" required /></div>
                <div><label className="block text-sm uppercase tracking-wide text-gray-700 mb-2">Açıklama (Opsiyonel)</label><input type="text" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Ek bilgi..." className="w-full border border-gray-300 px-4 py-3 focus:outline-none focus:border-gray-900" /></div>
              </div>
        ...
        <div className="bg-gradient-to-r from-green-500 to-green-600 text-white p-6 mb-6 border border-green-600">
          <div className="flex justify-between items-center">
            <div><p className="text-sm uppercase tracking-wide opacity-90">Toplam Gelir</p><p className="text-4xl font-light mt-2">€{grandTotal.toFixed(2)}</p><p className="text-sm opacity-75 mt-1">{transactions.length} işlem</p></div>
            ...
            dailySummaries.map((summary) => (
              <div key={summary.date} className="bg-white border border-gray-200">
                <div className="bg-gray-50 px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                  <div><h3 className="text-lg font-medium text-gray-900">{new Date(summary.date).toLocaleDateString('tr-TR', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</h3><p className="text-sm text-gray-600 mt-1">{summary.transactions.length} işlem</p></div>
                  <div className="text-right"><p className="text-sm text-gray-600">Gün Sonu Toplamı</p><p className="text-2xl font-semibold text-green-600">€{summary.total.toFixed(2)}</p></div>
                </div>
                    ...
                    <tbody>
                      {summary.transactions.map((transaction) => (
                        <tr key={transaction.id} className="border-b border-gray-100 hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 text-sm text-right font-medium text-green-600">€{Number(transaction.amount).toFixed(2)}</td>
